...
using namespace qiuan;
    ...
    CReadFile::CReadFile(std::string filename)
    : m_file(-1)
    , m_errno(0)
      , m_filesize(0)
{
...
int CReadFile::reset(std::string filename)
{
...
int CReadFile::read(std::string& out)
{
    if(m_file < 0)
    {
        return -1;
...
//////////////////////////////////////////////////////////////////////
    ...
    CWriteFile::CWriteFile(std::string filename)
    : m_file(NULL)
    , m_errno(0)
    , m_filesize(0)
      , m_writesize(0)
...
void CWriteFile::flush(uint64_t interval)
{
    CTime curtime;
...
int CWriteFile::reset(std::string filename)
{
    if(NULL != m_file)
    {
        flush();
...
ssize_t CWriteFile::write(const std::string& in)
{
    return write(in.c_str(), in.size());    
}

ssize_t CWriteFile::write(const char* inbuf, size_t inlen)
{
